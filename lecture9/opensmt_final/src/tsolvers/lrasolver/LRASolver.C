...
void LRASolver::addVarToRow( LAVar* s, LAVar* x, Real * p_v )
{
  ...
  if( x->isNonbasic( ) )
  {
    // Update the model of the Basic variable to comply with the new non-basic
    s->incM( *p_v * x->M( ) );
  ...
  for( unsigned in = 1 ; in < egraph.getNofPartitions( ) ; in++ )
  {
    ...
    for( unsigned i = 0 ; i < explanation.size( ) ; i++ )
    {
      ...
      if( color == I_A )
      {
        ...
        {
          interpolant.addExprWithCoeff( LAExpression( explanation[ i ] )
	                              , explanationCoefficients[ i ] );
        }
        else
        {
          interpolant.addExprWithCoeff( LAExpression( explanation[ i ] )
