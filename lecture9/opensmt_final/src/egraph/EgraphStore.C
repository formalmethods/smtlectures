...
#define SIMPLIFY_PLUS 1          // Optimization for plus. May be memory-expensive
...
void Egraph::initializeStore( )
{
  ...
  Snode * sparC1      = sort_store.mkPara( "_C_" );
  Snode * sparC1_list = sort_store.cons( sparC1 );
  Snode * sparC2_list = sort_store.cons( sparC1, sparC1_list );
  Snode * sparC2      = sort_store.mkDot( sparC2_list );

  Snode * sarray1 = sort_store.mkVar( "Array" );
  //
  ...
  //
  newSymbol_( "true"  , NULL, sbool1 ); assert( ENODE_ID_TRUE    == id_to_enode.size( ) - 1 );
  newSymbol_( "false" , NULL, sbool1 ); assert( ENODE_ID_FALSE   == id_to_enode.size( ) - 1 );
  //
  // Core
  //
  newSymbol_( "not"     , sbool1     , sbool1 ); assert( ENODE_ID_NOT      == id_to_enode.size( ) - 1 );
  newSymbol_( "=>"      , sbool2     , sbool1 ); assert( ENODE_ID_IMPLIES  == id_to_enode.size( ) - 1 );
  newSymbol_( "and"     , sbool2     , sbool1 ); assert( ENODE_ID_AND      == id_to_enode.size( ) - 1 );
  newSymbol_( "or"      , sbool2     , sbool1 ); assert( ENODE_ID_OR       == id_to_enode.size( ) - 1 );
  newSymbol_( "xor"     , sbool2     , sbool1 ); assert( ENODE_ID_XOR      == id_to_enode.size( ) - 1 );
  newSymbol_( "="       , sparA2     , sbool1 ); assert( ENODE_ID_EQ       == id_to_enode.size( ) - 1 );
  newSymbol_( "ite"     , sbool_parA2, sparA1 ); assert( ENODE_ID_ITE      == id_to_enode.size( ) - 1 );
  newSymbol_( "distinct", sparA2     , sbool1 ); assert( ENODE_ID_DISTINCT == id_to_enode.size( ) - 1 );
  //
  ...
  newSymbol_( "+"       , sarith2, sarith1 ); assert( ENODE_ID_PLUS   == id_to_enode.size( ) - 1 );
  newSymbol_( "-"       , sarith2, sarith1 ); assert( ENODE_ID_MINUS  == id_to_enode.size( ) - 1 );
  newSymbol_( "-"       , sarith1, sarith1 ); assert( ENODE_ID_UMINUS == id_to_enode.size( ) - 1 );
  newSymbol_( "*"       , sarith2, sarith1 ); assert( ENODE_ID_TIMES  == id_to_enode.size( ) - 1 );
  newSymbol_( "/"       , sarith2, sarith1 ); assert( ENODE_ID_DIV    == id_to_enode.size( ) - 1 );
  newSymbol_( "<="      , sarith2, sbool1 );  assert( ENODE_ID_LEQ    == id_to_enode.size( ) - 1 );
  newSymbol_( ">="      , sarith2, sbool1 );  assert( ENODE_ID_GEQ    == id_to_enode.size( ) - 1 );
  newSymbol_( "<"       , sarith2, sbool1 );  assert( ENODE_ID_LT     == id_to_enode.size( ) - 1 );
  newSymbol_( ">"       , sarith2, sbool1 );  assert( ENODE_ID_GT     == id_to_enode.size( ) - 1 );
  ...
  newSymbol_( "store"   , sarray_parA_parB__parA_parB, sarray_parA_parB ); assert( ENODE_ID_STORE  == id_to_enode.size( ) - 1 );
  newSymbol_( "select"  , sarray_parA_parB__parA, sparB1 );                assert( ENODE_ID_SELECT == id_to_enode.size( ) - 1 );
  //
  // Cost theory symbols
  //
  ...
  newSymbol_( "incur", sct_incur, sbool1 ); assert( ENODE_ID_CTINCUR == id_to_enode.size( ) - 1 );
  newSymbol_( "bound", sct_bound, sbool1 ); assert( ENODE_ID_CTBOUND == id_to_enode.size( ) - 1 );

  Snode * sarray_parA_parB_array_parA_parB = sort_store.mkDot( sort_store.cons( sarray_parA_parB
	                                                     , sort_store.cons( sarray_parA_parB ) ) );

  newSymbol_( "diff", sarray_parA_parB_array_parA_parB, sparA1 ); assert( ENODE_ID_DIFF == id_to_enode.size( ) - 1 );

  newSymbol_( "fake_interp", NULL, sbool1 ); assert( ENODE_ID_FAKE_INTERP == id_to_enode.size( ) - 1 );

  // Recursive Datatypes Data Structures
  newSymbol_( "car" , sparC1, sparC1 ); assert( ENODE_ID_CAR  == id_to_enode.size( ) - 1 );
  newSymbol_( "cdr" , sparC1, sparC1 ); assert( ENODE_ID_CDR  == id_to_enode.size( ) - 1 );
  newSymbol_( "cons", sparC2, sparC1 ); assert( ENODE_ID_CONS == id_to_enode.size( ) - 1 );
  //
...
Enode * Egraph::cons( vector< Enode * > & args )
{
  Enode * elist = const_cast< Enode * >( enil );

  for ( vector< Enode * >::iterator it = args.begin( )
      ; it != args.end( )
      ; it ++ )
    elist = cons( *it, elist );
...
Enode * Egraph::mkVar( const char * name, bool model_var )
{
  ...
  // It's a variable
  Enode * res = cons( e );
  if ( model_var || config.produce_models != 0 )
...
Enode * Egraph::mkDefineFun( DefineFun & df, Enode * args )
{
  assert( args );
  assert( args->isList( ) );
  //
  // Fill substitution map
  //
  map< Enode *, Enode * > substs;
  unsigned i = 0;
  for ( ; !args->isEnil( ) ; args = args->getCdr( ) )
  {
    Enode * arg = args->getCar( );
    substs[ df.getArg( i ) ] = arg;
    i ++;
  }
  //
  // Perform substitution and return
  // 
  return substitute( df.getDefine( ), substs );
}
...
Enode * Egraph::mkFun( const char * name, Enode * args )
{
  ...
  assert( args->isList( ) );
  //
  // Check if it is a define-fun
  //
  map< string, DefineFun * >::iterator it = name_to_define_fun.find( name );
  if ( it != name_to_define_fun.end( ) )
    return mkDefineFun( *it->second, args );
  //
...
Enode *
Egraph::mkRDCons( Enode * car, Enode * cdr )
{
  assert( car );
  assert( cdr );
  return cons( id_to_enode[ ENODE_ID_CONS ], cons( car, cons( cdr ) ) );
}

Enode *
Egraph::mkRDCar( Enode * x )
{
  assert( x );
  return cons( id_to_enode[ ENODE_ID_CAR ], cons( x ) );
}

Enode *
Egraph::mkRDCdr( Enode * x )
{
  assert( x );
  return cons( id_to_enode[ ENODE_ID_CDR ], cons( x ) );
}

// 
// Check if symbol has been declared
//
bool Egraph::hasSymbol( const char * name, Snode * arg, Snode * ret )
{
  assert( ret );
  assert( ret->isTerm( ) );
  (void)ret;
...
//
Enode * Egraph::newSymbolLog( const char * name
                            , Snode * args          // Arguments sorts (only for arity > 0, NULL otherwise)
                            , Snode * retv          // Return sort
			    , uint64_t p )          // Parameter for properties, not used now
{
  if ( config.dump_log )
  {
  ...
  return newSymbol_( name, args, retv, p );
}
...
//
// Creates a new symbol. Name must be new -- does not report to log
//
Enode * Egraph::newSymbol_( const char * name
                          , Snode * args
			  , Snode * retv
			  , uint64_t )
{
...
void Egraph::newDefine( const char * name
                      , Enode * var_list
		      , Enode * fun )
{
  assert( name );
  assert( var_list );
  assert( fun );
  assert( var_list->isList( ) );
  assert( fun->isTerm( ) );

  // TODO: should check that name is unique also w.r.t.
  // uninterpreted functions, variables, etc.
  if ( name_to_define_fun.find( name ) != name_to_define_fun.end( ) )
    opensmt_error2( "define already declared ", name );

  DefineFun * d = new DefineFun( name, var_list, fun );

  name_to_define_fun[ name ] = d;
}
...
//
// Remember, this to support lets, its not the smtlib define
//
...
Enode * Egraph::mkSelect( Enode * a, Enode * i )
{
  ...
  if ( symb == NULL )
  {
    Snode * saie_i = sort_store.mkDot( sort_store.cons( as
	                             , sort_store.cons( is ) ) );
    symb = newSymbol_( "select", saie_i, es ); 
  ...
  // Prints out array value if required
  if ( config.produce_models != 0 )
    variables.insert( newSel );
...
Enode * Egraph::mkStore( Enode * a, Enode * i, Enode * e )
{
  ...
  if ( symb == NULL )
  {
    Snode * saie_i_e = sort_store.mkDot( sort_store.cons( as
	                               , sort_store.cons( is
				       , sort_store.cons( es ) ) ) );
    symb = newSymbol_( "store", saie_i_e, as ); 
...
Enode * Egraph::mkPlus( Enode * args )
{
  ...
  //
	 ...
	 && ( x->get1st( )->isConstant( ) 
	   || x->get2nd( )->isConstant( ) ) )
  {
    Enode * x_term = x->get1st( )->isConstant( ) ? x->get2nd( ) : x->get1st( );
    Enode * x_cons = x_term == x->get1st( ) ? x->get2nd( ) : x->get1st( ); 
    const Real & xval = x_cons->getValue( );
    const Real & yval = y->getValue( );
    Real sum = xval + yval;
    Enode * enode_sum = mkNum( sum );
    Enode * new_args = cons( x_term, cons( enode_sum ) );
    res = cons( id_to_enode[ ENODE_ID_PLUS ], new_args );
  }
#endif
  else
  {
...
{
  assert( args );
  assert( args->isList( ) || args->isTerm( ) );
  // Extract arg from list, if args is list
  Enode * arg = args->isList( ) 
              ? args->getCar( )
	      : args ;
  // Construct list from arg, if args is term
  args = args->isList( ) 
       ? args 
       : cons( arg ) ;
...
Enode * Egraph::mkAnd( Enode * args )
{
  ...
  if ( config.e_simpl_level > 0 )
  {
    initDup1( );

    vector< Enode * > conjuncts;
    for ( Enode * alist = args 
	; !alist->isEnil( ) 
	; alist = alist->getCdr( ) )
    {
      ...
      // Unroll a
      unroll.push_back( e );
      // Loop until there is nothing left to unroll
      while( !unroll.empty( ) )
      {
	Enode * conj = unroll.back( );
	unroll.pop_back( );

	if ( isDup1( conj ) ) continue;
	if ( conj->isTrue( ) ) continue;
	if ( conj->isFalse( ) ) { doneDup1( ); return mkFalse( ); }
	// Push children to be unrolled
	if ( conj->isAnd( ) )
	{
	  for ( Enode * ulist = conj->getCdr( ) 
	      ; !ulist->isEnil( ) 
	      ; ulist = ulist->getCdr( ) )
	  {
	    Enode * f = ulist->getCar( );
	    unroll.push_back( f );
	  }
	}
	// Push child in the list of new arguments
	else
	{
  ...
  // Default duplicate removal only
  else
  {
    initDup1( );
...
Enode * Egraph::mkIff( Enode * args )
{
  ...
  // Simplify (iff (not a) (not b)) to (iff a b)
  if ( first->isNot( ) && second->isNot( ) )
  {
    first = first->get1st( );
    second = second->get1st( );
  }
  ...
  // Make sure that if there is a variable it is in first
  if ( !first->isVar( ) )
  {
    Enode * tmp = first;
    first = second;
    second = tmp;
    args = cons( first, cons( second ) );
  }

  /*
  return mkAnd( cons( mkImplies( cons( first, cons( second ) ) )
              , cons( mkImplies( cons( second, cons( first ) ) ) ) ) 
              );
  */
...
Enode * Egraph::mkXor( Enode * args )
{
  ...
  return mkIff( cons( first, cons( mkNot( second ) ) ) );

  // Rewrite (xor a b) as (iff a (not b))
  /*
...
Enode * Egraph::mkExtract( int msb, int lsb, Enode * arg )
{
  ...
  else
  {
    ...
    if ( it == ext_store.end( ) )
    {
      ...
      e = newSymbol_( name, DTYPE_BITVEC | (msb - lsb + 1) );
      e->setExtract( lsb );
      ext_store[ sig ] = e;
    }
    else
...
Enode * Egraph::mkSignExtend( int i, Enode * x )
{
  ...
  else
  {
	...
	se_store.resize( i + 1, NULL );
      ...
      assert( lookupSymbol( name ) == NULL );
      e = newSymbol_( name, DTYPE_BITVEC | (i + x->getWidth( )) );
      se_store[ i ] = e;
    }
...
Enode * 
Egraph::substitute ( Enode * formula 
                   ...
                   , vector< Enode * > & new_vars )
{
  ...
  for ( size_t i = 0 ; i < old_vars.size( ) ; i ++ )
    storeDupMap1( old_vars[ i ], new_vars[ i ] );
  ...
  while( !unprocessed_enodes.empty( ) )
  {
    ...
    Enode * result = copyEnodeEtypeTermWithCache( e );

    storeDupMap1( e, result );
  }

  assert( valDupMap1( formula ) != NULL );
  Enode * res = valDupMap1( formula );
...
#ifdef PRODUCE_PROOF
Enode *
Egraph::safeSubstitute( Enode * formula, map< Enode *, Enode * > & substs )
{
  assert( formula );
  vector< Enode * > unprocessed_enodes;
  initDupMap1( );
  ...
  while( !unprocessed_enodes.empty( ) )
  {
    ...
    if ( result->isTimes( )
      && !result->get1st( )->isConstant( ) 
      && !result->get2nd( )->isConstant( ) )
    {
      // Creating a non-linear term, abort
      // doneDupMap1( );
      // return NULL;
      // Creating a non-linear term, restore prev
      result = enode;
    }
...
#endif
//
// Maximizes the colors of this formula, i.e.,
// extends the color of uninterpreted symbols
// to interpreted symbols. Excexpt for boolean
// symbols, which will be taken care of by
// the cnfizer
//
void Egraph::maximizeColors( Enode * formula )
{
  assert( config.produce_inter != 0 );
  vector< Enode * > unprocessed_enodes;
  unprocessed_enodes.push_back( formula );
  ...
  while( !unprocessed_enodes.empty( ) )
  {
	...
	max_inner_parts &= arg->getIPartitions( );
      ...
      undo_stack_part.push_back( enode->getIPartitions( ) );
      undo_stack_node.push_back( enode );
...
//
// Now we need to color predicates. Notice that predicates
// like x=y (or any other predicate symbol), with x and y AB-common 
// are not AB-commom, but they are colored with "partition"
//
void Egraph::finalizeColors( Enode * f, const ipartitions_t & partition )
{
  // Maximizes colors first
  maximizeColors( f );
  vector< Enode * > unprocessed_enodes;
...
Enode * Egraph::prepareAssertion( Enode * e, const bool neg )
{
  assert( e );
  ...
  if ( config.incremental )
  {
    ...
    if ( config.logic == QF_IDL
      ...
      || config.logic == QF_UFIDL
      || config.logic == QF_UFLRA 
      || config.logic == QF_UFLIA
      || config.logic == QF_AXLIA
      )
    {
      if ( config.sat_lazy_dtc != 0 )
	e = canonizeDTC( e, config.split_equalities );
      else
	e = canonize( e, config.split_equalities );
    }
    //
    ...
    {
      opensmt_warning( "assumption not canonized/normalized" );
    }
  }
...
#ifdef PRODUCE_PROOF
void Egraph::colorAsShared( Enode * e )
{
  assert( e );
  assert( config.produce_inter != 0 );
  // Set all global variables
  ipartitions_t partitions = -2;
  colorUF( e, partitions );
}

Enode * Egraph::preparePartition( Enode * e, const ipartitions_t & p )
{
  assert( config.produce_inter != 0 );
  Enode * res = prepareAssertion( e ); // Normally process
  colorUF( res, p );                   // Color variables and uninterpreted symbols
  return res;
}

// Color uninterpreted symbols in formula
void Egraph::colorUF( Enode * formula, const ipartitions_t & partitions )
{
  assert( config.produce_inter != 0 );
  ...
  while( !unprocessed_enodes.empty( ) )
  {
    ...
    // Color uninterpreted symbols
    // Terms made with these symbols
    // will be colored in finalizeColors
    if ( enode->isUf( ) 
      || enode->isUp( ) )
    {
      undo_stack_part.push_back( enode->getCar( )->getIPartitions( ) );
      undo_stack_node.push_back( enode->getCar( ) );

      enode->getCar( )->addIPartitions( partitions );
    }
    // Color variables at this stage
    // Alternatively could be done later with
    // finalizeColors
    if ( enode->isVar( ) )
    {
      undo_stack_part.push_back( enode->getIPartitions( ) );
...
void Egraph::saveColors( )
{
  assert( config.produce_inter != 0 );
  assert( undo_stack_part.size( ) == undo_stack_node.size( ) );
  bckt_stack_part.push_back( undo_stack_part.size( ) );
}

void Egraph::restoreColors( )
{
  assert( config.produce_inter != 0 );
  const size_t new_size = bckt_stack_part.back( );
  bckt_stack_part.pop_back( );
  assert( new_size <= undo_stack_part.size( ) );
  while ( new_size < undo_stack_part.size( ) )
  {
    Enode * e = undo_stack_node.back( );
    undo_stack_node.pop_back( );
    ipartitions_t p = undo_stack_part.back( );
    undo_stack_part.pop_back( );
    // Restore old partitions
    e->setIPartitions( p );
  }
  assert( new_size == undo_stack_part.size( ) );
}
...
Enode * Egraph::absorbOr( Enode * formula )
{
  assert( formula );
  ...
  while( !unprocessed_enodes.empty( ) )
  {
    ...
    // Change (or a b) into (not (and (not a) (not b)))
    // and see if it can be changed to some
    // (not c)
    if ( e->isOr( ) )
    {
      // Only two patterns recognized at the moment:
      // 1. (or (<= (+ a b) -1) (<= (+ b a) -1))         --> (not (= a b))
      // 2. (or (and (<= (- a b) 0) (<= (- b c) -1) ...) --> (not (= a c))
      if ( e->getArity( ) == 2 )
      {
	Enode * arg1 = e->get1st( );
	Enode * arg2 = e->get2nd( );
	...
	// Case 2
	      ...
	      && arg2->isAnd( ) )
	{
	  result = absorbLeqChain( arg1, arg2 );
	}
	*/
      }
    }
    ...
    if ( result == NULL )
      result = copyEnodeEtypeTermWithCache( e );
...
Enode * Egraph::absorbLeq( Enode * leq1, Enode * leq2 )
{
  LAExpression leq1_can_exp( leq1 );
  LAExpression leq2_can_exp( leq2 );
  Enode * leq1_can = leq1_can_exp.toEnode( *this );
  Enode * leq2_can = leq2_can_exp.toEnode( *this );

  // Case 1: both are simple bounds
  if ( leq1_can->get1st( )->isTimes( )
    && leq2_can->get1st( )->isTimes( ) 
    && leq1_can->get2nd( )->getValue( ) == -1
    && leq2_can->get2nd( )->getValue( ) == -1 
    && leq1_can->get1st( )->get1st( )->getValue( ) ==  1 
    && leq2_can->get1st( )->get1st( )->getValue( ) == -1 
    )
  {
    Enode * v = leq1_can->get1st( )->get2nd( );
    // Fail
    if ( v != leq2_can->get1st( )->get2nd( ) ) return NULL;
    return mkNot( mkEq( v, mkNum( "0" ) ) );
  }

  if ( !leq1->get1st( )->isPlus( ) ) return NULL;
  if ( !leq2->get1st( )->isPlus( ) ) return NULL;

  assert( leq1->get1st( )->isPlus( ) );
  assert( leq2->get1st( )->isPlus( ) );

  if ( leq1->get2nd( )->getValue( ) != -1 ) return NULL;
  if ( leq2->get2nd( )->getValue( ) != -1 ) return NULL;

  Enode * a1  = leq1->get1st( )->get1st( );
  Enode * b1_ = leq1->get1st( )->get2nd( );

  // cerr << " a1: " << a1 << endl;
  // cerr << "b1_: " << b1_ << endl;

  if ( !b1_->isTimes( ) ) return NULL;
  if ( b1_->get1st( )->getValue( ) != -1 ) return NULL;

  Enode * b1 = b1_->get2nd( );

  Enode * a2  = leq2->get1st( )->get1st( );
  Enode * b2_ = leq2->get1st( )->get2nd( );

  if ( !b2_->isTimes( ) ) return NULL; 
  if ( b2_->get1st( )->getValue( ) != -1 ) return NULL;

  Enode * b2 = b2_->get2nd( );

  // Finally, there is a match, return a1!=a2
  if ( a1 == b2 && a2 == b1 )
    return mkNot( mkEq( a1, a2 ) );

  return NULL;
}

Enode * Egraph::absorbLeqChain( Enode * and1, Enode * and2 )
{
  assert( and1 );
  assert( and2 );

  if ( !and1->isAnd( ) ) return NULL;
  if ( !and2->isAnd( ) ) return NULL;
  //
  // Processing first AND
  //
  Enode * l11 = and1->get1st( );
  Enode * l12 = and1->get2nd( );

  // We now retrieve this
  // (<= (- a11 b11) 0) (<= (- a12 b12) -1)
  // (<= (- a21 b21) 0) (<= (- a22 b22) -1)

  if ( !l11->isLeq( ) ) return NULL;
  if ( !l12->isLeq( ) ) return NULL;

  Enode * plus11 = l11->get1st( );
  Enode * numb11 = l11->get2nd( );

  if ( !plus11->isPlus( ) ) return NULL;
  if ( numb11->getValue( ) != -1 
    && numb11->getValue( ) != 0 )
    return NULL;

  Enode * a11  = plus11->get1st( );
  Enode * b11_ = plus11->get2nd( );

  if ( !b11_->isTimes( ) ) return NULL;
  if ( b11_->get1st( )->getValue( ) != -1 ) return NULL;

  Enode * b11 = b11_->get2nd( );

  Enode * plus12 = l12->get1st( );
  Enode * numb12 = l12->get2nd( );

  if ( !plus12->isPlus( ) ) return NULL;
  if ( numb12->getValue( ) != -1 
    && numb12->getValue( ) != 0 )
    return NULL;

  Enode * a12  = plus12->get1st( );
  Enode * b12_ = plus12->get2nd( );

  if ( !b12_->isTimes( ) ) return NULL;
  if ( b12_->get1st( )->getValue( ) != -1 ) return NULL;

  Enode * b12 = b12_->get2nd( );

  if ( a11 != b12 && a12 != b11 ) return NULL;
  //
  // Processing second AND
  //
  Enode * l21 = and2->get1st( );
  Enode * l22 = and2->get2nd( );

  if ( !l21->isLeq( ) ) return NULL;
  if ( !l22->isLeq( ) ) return NULL;

  Enode * plus21 = l21->get1st( );
  Enode * numb21 = l21->get2nd( );

  if ( !plus21->isPlus( ) ) return NULL;
  if ( numb21->getValue( ) != -1 
    && numb21->getValue( ) != 0 )
    return NULL;

  Enode * a21  = plus21->get1st( );
  Enode * b21_ = plus21->get2nd( );

  if ( !b21_->isTimes( ) ) return NULL;
  if ( b21_->get1st( )->getValue( ) != -1 ) return NULL;

  // Enode * b21 = b21_->get2nd( );

  Enode * plus22 = l22->get1st( );
  Enode * numb22 = l22->get2nd( );

  if ( !plus22->isPlus( ) ) return NULL;
  if ( numb22->getValue( ) != -1 
    && numb22->getValue( ) != 0 )
    return NULL;

  // Enode * a22  = plus22->get1st( );
  Enode * b22_ = plus22->get2nd( );

  if ( !b22_->isTimes( ) ) return NULL;
  if ( b22_->get1st( )->getValue( ) != -1 ) return NULL;

  Enode * b22 = b22_->get2nd( );

  if ( a11 != a21 && a11 != b22 ) return NULL;
  if ( b12 != a21 && b12 != b22 ) return NULL;
  if ( a11 == b12 ) return NULL;

  // Now we try to figure out the two variables
  if ( a11 == a21 ) return mkNot( mkEq( a11, b22 ) );
  if ( a11 == b22 ) return mkNot( mkEq( a11, a21 ) );
