...
public:
  ...
  // inline bool hasSortArray       ( ) { assert( isTerm( ) ); Snode * last = getLast( ); assert( last->getArity( ) == 3 ); return last->get1st( )->hasSymbolId ( SNODE_ID_ARRAY ); }
  ...
  inline bool isNat              ( ) { assert( isTerm( ) ); return hasSymbolId( SNODE_ID_NAT ); }
  inline bool isInt              ( ) { assert( isTerm( ) ); return hasSymbolId( SNODE_ID_INT ); }
  inline bool isBool             ( ) { assert( isTerm( ) ); return hasSymbolId( SNODE_ID_BOOL ); }
  inline bool isReal             ( ) { assert( isTerm( ) ); return hasSymbolId( SNODE_ID_REAL ); }
  inline bool isArray            ( ) { assert( isTerm( ) ); if ( getArity( ) < 3 ) return false; assert( getArity( ) == 3 ); return get1st( )->hasSymbolId( SNODE_ID_ARRAY ); }
  inline bool isUndef            ( ) { assert( isTerm( ) ); return hasSymbolId( SNODE_ID_UNDEF ); }
