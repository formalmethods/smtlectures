...
#define opensmt_error( S )        { cerr << "# OpenSMT Error: " << S << " (triggered at " <<  __FILE__ << ", " << __LINE__ << ")" << endl; exit( 1 ); }
#define opensmt_error2( S, T )    { cerr << "# OpenSMT Error: " << S << " " << T << " (triggered at " <<  __FILE__ << ", " << __LINE__ << ")" << endl; exit( 1 ); }
#define opensmt_warning( S )      { cerr << "# OpenSMT Warning: " << S << endl; }
#define opensmt_warning2( S, T )  { cerr << "# OpenSMT Warning: " << S << " " << T << endl; }
#define mcmt_error( S )           { cerr << "# MCMT Error: " << S << " (triggered at " <<  __FILE__ << ", " << __LINE__ << ")" << endl; exit( 1 ); }
#define mcmt_error2( S, T )       { cerr << "# MCMT Error: " << S << " " << T << " (triggered at " <<  __FILE__ << ", " << __LINE__ << ")" << endl; exit( 1 ); }
#define mcmt_warning( S )         { cerr << "# MCMT Warning: " << S << endl; }
#define mcmt_warning2( S, T )     { cerr << "# MCMT Warning: " << S << " " << T << endl; }
...
#define FIRST                   1
#define SECOND                  2
#define MIXED_STRATEGY          0
#define DEPTH_FIRST_STRATEGY    1
#define BREADTH_FIRST_STRATEGY  2

#define MAX_INDEX_VARS          10
...
typedef enum
{
  ...
  , QF_CT	  // Cost
  , QF_AXLIA      // AX + LIA. Logic for MCMT2
  , QF_RD         // Recursive Datatypes
  , QF_SO         // Strict Orders
...
// For mcmt
#define ARR_FMT2 "%s[%s]"
#define ARR_FMT3 "%s[%s]::%s"
#define ARR_INST( A, I ) A << "[" << I << "]"
...
inline const char * logicStr ( logic_t l )
{
  ...
  else if ( l == QF_AXLIA )  return "QF_AXLIA";
  else if ( l == QF_RD )     return "QF_RD";
...
using opensmt::QF_CT;       
using opensmt::QF_AXLIA;
using opensmt::QF_RD;
using opensmt::QF_SO;
using opensmt::cpuTime;       
using opensmt::memUsed;       
using opensmt::logicStr;       
...
using mcmt::SAFE;       
using mcmt::UNSAFE;       
using mcmt::OPENSMT_SOLVER;
using mcmt::YICES_SOLVER;

using oprover::tool_t;
using oprover::TOOL_UNDEF;
using oprover::OPENSMT;
using oprover::MCMT;
