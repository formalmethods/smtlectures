...
bool Egraph::checkCanonized( Enode * e )
{
  if ( e->isDistinct( ) )
    return true;
  assert( config.split_equalities != 1 || e->isLeq( ) );
  assert( config.split_equalities == 1 || e->isLeq( ) || e->isEq( ) );
  LAExpression la( e );
  return la.toEnode( *this ) == e;
}

#ifdef PRODUCE_PROOF
void Egraph::showColors( Enode * formula )
{
  assert( formula );
  ...
  while( !unprocessed_enodes.empty( ) )
  {
    ...
    cerr << enode << " " << enode->getIPartitions( ) << endl;
