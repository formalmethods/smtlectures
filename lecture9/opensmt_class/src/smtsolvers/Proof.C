...
void CoreSMTSolver::dumpRndInter( )
{
 ...
 Proof::Proof( CoreSMTSolver & solver_ )
  : solver    ( solver_ )
  , begun     ( false )
  , chain_cla ( NULL )
  , chain_var ( NULL )
...
void Proof::endChain( Clause * res )
{
  ...
  /*
  cerr << "Storing for : " << res << endl;
  cerr << "  "; 
  if ( res )
    solver.printSMTClause( cerr, *res );
  else
    cerr << "NULL";
  cerr << endl;
  */
...
void Proof::reset( ) 
{ 
  map< Clause *, ProofDer * >::iterator it = clause_to_proof_der.find( NULL );
...
void CoreSMTSolver::getInter( vector< Enode * > & interpolants )
{
  assert( config.produce_inter != 0 );
  ...
  graph.produceSequenceInterpolants( interpolants );

  if ( config.proof_certify_inter > 0 )
  {
    if ( config.verbosity > 1 )
      cerr << "# Certifying interpolant ... ";
    verifyInterpolantWithExternalTool( interpolants );
    if ( config.verbosity > 1 )
      cerr << "OK" << endl;
  }
}
