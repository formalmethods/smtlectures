...
%token TK_PUSH TK_POP TK_CHECKSAT TK_GETASSERTIONS TK_GETPROOF TK_GETUNSATCORE TK_GETINTERPOLANTS TK_GETINTERPOLANT
...
%token TK_ASSERT TK_ASSERT_PARTITION
...
%token TK_STORE TK_SELECT TK_DIFF TK_SPLITEQUALITIES TK_GETMODEL
%token TK_CONS TK_CAR TK_CDR
...
command: '(' TK_SETLOGIC symbol ')'
	 ...
	 { 
	   parser_ctx->mkBind( $3, $7 ); free( $3 );
         }
       | '(' TK_PUSH numeral ')'
	 { parser_ctx->Push( atoi( $3 ) ); free( $3 ); }
       | '(' TK_POP numeral ')'
	 { parser_ctx->Pop( atoi( $3 ) ); free( $3 );}
       | '(' TK_ASSERT term ')'
         { parser_ctx->Assert( $3 ); }
       | '(' TK_ASSERT_PARTITION term ')'
         { parser_ctx->AssertPartition( $3 ); }
       | '(' TK_CHECKSAT ')'
	 { parser_ctx->CheckSAT( ); }
       /*
       | '(' TK_GETASSERTIONS ')'
	 ...
	 { parser_ctx->GetProof( ); }
       | '(' TK_GETINTERPOLANTS ')'
	 { parser_ctx->GetInterpolants( ); }
       | '(' TK_GETINTERPOLANT numeral ')'
	 { parser_ctx->GetInterpolant( atoi( $3 ), true ); free( $3 ); }
       /*
       | '(' TK_GETUNSATCORE ')'
	 { opensmt_error2( "command not supported (yet)", "" ); }
...
sort: TK_BOOL 
    ...
    | '(' TK_ARRAY sort sort ')'
      { 
	Snode * a = parser_ctx->mkSortVar( "Array" );
	$$ = parser_ctx->mkSort( 
                 parser_ctx->mkCons( a
               , parser_ctx->mkCons( $3
	       , parser_ctx->mkCons( $4 ) ) ) ); 
...
term: spec_const 
   ...
   */
    ...
    | '(' TK_CONS term term ')'
      { $$ = parser_ctx->mkRDCons( $3, $4 ); }
    | '(' TK_CDR term ')'
      { $$ = parser_ctx->mkRDCdr( $3 ); }
    | '(' TK_CAR term ')'
      { $$ = parser_ctx->mkRDCar( $3 ); }
    | '(' TK_DIFF term term ')'
...
option: TK_PRINT_SUCCESS b_value
      ...
      | TK_VERBOSITY TK_NUM
	{
      ...
      | TK_SPLITEQUALITIES b_value
	{
	  parser_ctx->SetOption( ":split-equalities", $2 );
          free( $2 );
	}
      | TK_KEY
