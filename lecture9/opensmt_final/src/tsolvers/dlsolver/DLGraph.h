...
template <class T> struct DLEdge 
{
  ...
  inline void printDLEdge( ostream &os ) 
  { 
    if ( u->e ) os << u->e; else os << "NULL";
    os << " --[" << wt << "]--> "; 
    if ( v->e ) os << v->e; else os << "NULL";
  }
...
public:
    ...
    DLGraph( Config & config_, Egraph & egraph_ ) 
      : Vcnt		   ( 0 ) 
      , Ecnt		   ( 0 )
  ...
  void	      deleteActive  ( Enode *, const bool );
  ...
  bool        checkNegCycle      ( Enode *, const bool );
  bool        checkNegCycleEq    ( Enode *, const bool );
  bool        checkNegCycleNeq   ( vector< Enode * > & );
...
private:
  ...
  bool        checkNegCycle_     ( DLEdge<T> * );
  void        storeNegCycle      ( vector< Enode * > &, set< Enode * > & );
#ifdef PRODUCE_PROOF
  Enode *     mergeInterpolants  ( Enode *, Enode *, Enode * );
  void        computeInterpolants( );
#endif

  DLEdge<T> * insertDynamic ( Enode * );
  DLEdge<T> * insertDynamic ( Enode *, const bool );
  ...
  vector< Enode * > ne_stack;            // Negated equalities stack
  vector< size_t >  ne_stack_backtrack;  // Ne backtrack stack
