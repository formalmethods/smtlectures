...
Enode::Enode( )
  ...
  , mcmt_id         ( 0 )
  , mcmt_global     ( false )
  , mcmt_abstracted ( false )
  , mcmt_index_var  ( false )
#endif
  , properties ( 0 )
  , car        ( NULL )
  , cdr        ( NULL )
...
#endif
{
  ...
  assert( !isTerm( ) || car );
  assert( !isTerm( ) || car->symb_data );
  assert( !isTerm( ) || car->symb_data->ret_sort );
...
// 
// MCMT related printing routines
//
#ifndef SMTCOMP
void Enode::yicesPrint( ostream & os )
{
  Enode * p = NULL;
  ...
  else if ( isEnil( ) )
  {
    ...
    mcmt_error( "unknown case value" );
}

void Enode::latexPrint( ostream & os )
{
  Enode * p = NULL;
