...
{
  return inform( e, true );
}

lbool Egraph::inform( Enode * e, const bool use_tsolvers )
{
  assert( e );
  assert( tsolvers_init );
  ...
  if ( it == informed.end( ) )
  {
    ...
    // Skip this step if opensmt is not used by MCMT
    if ( use_tsolvers )
    {
      ...
      {
	bool unassigned_atom = ( config.split_equalities == 1 && e->isLeq( ) )
	                    || ( config.split_equalities == 0 && ( e->isLeq( ) 
				                                || e->isEq ( ) ));  // Arithmetic should be assigned

	// assert( checkCanonized( e ) );
...
bool Egraph::assertLit( Enode * e )
{
  ...
  */
  ...
  const bool res = assertLit( e, true, false );
...
bool Egraph::assertLit( Enode * e, const bool reason )
{
  return assertLit( e, true, reason );
}

bool Egraph::assertLit( Enode * e, const bool use_tsolvers, const bool reason )
{
  /*
...
bool Egraph::check( bool complete )
{
  ...
  if ( !res )
  {
    ...
    for ( size_t i = 0 ; i < explanation.size( ) ; i ++ )
      cerr << " " 
	   << ( explanation[ i ]->getPolarity( ) == l_False ? "not " : "    " ) 
	   << explanation[ i ] 
	   << endl;

    for ( size_t i = 0 ; i < explanation.size( ) ; i ++ )
    {
      cerr << "COLORS FOR : " << explanation[ i ] << endl;
      showColors( explanation[ i ] );
    }
  }
  */
...
#ifdef PRODUCE_PROOF
//
// Parameter l is an output parameter needed
// to communicate the logic for certify interpolants 
//
...
void Egraph::initializeTheorySolvers( )
{
  ...
  else if ( config.logic == QF_RDL )
  {
    ...
    else
    {
      ...
      {
	if ( getUseGmp( ) || config.incremental )
  ...
  else if ( config.logic == QF_AXLIA )
  {
    ...
    tsolvers_stats.push_back( new TSolverStats( ) );
#endif
    // Allocate LRA to deal with additional complexity of computing interpolants
#ifdef PRODUCE_PROOF
    tsolvers      .push_back( new LRASolver( tsolvers.size( ), "LRA Solver", config, *this, sort_store, explanation, deductions, suggestions ) );
#ifdef STATISTICS
    tsolvers_stats.push_back( new TSolverStats( ) );
  ...
  else if ( config.logic == QF_RD )
  {
    // Allocating a cost solver (we always do this, unless in QF_UF)
    tsolvers.push_back( new RDSolver( tsolvers.size( ), "Recursive Datatype Solver", config, *this, sort_store, explanation, deductions, suggestions ) );
#ifdef STATISTICS
    tsolvers_stats.push_back( new TSolverStats( ) );
#endif
  }
...
void Egraph::backtrackToStackSize ( size_t size )
{
  ...
  while ( undo_stack_term.size( ) > size )
  {
	...
	car->removeParent( e );
      ...
      removeStore( e );
    else if ( last_action == INTERFACE_TERM )
    {
      interface_terms_cache.erase( e );
      interface_terms.pop_back( );
    }
    else if ( last_action == INTERFACE_LA )
      it_la.erase( e );
    else if ( last_action == INTERFACE_UF )
      it_uf.erase( e );
    else if ( last_action == ACK_APPL )
    {
      assert( uf_to_appl.find( e->getCar( ) ) != uf_to_appl.end( ) );
      assert( uf_to_appl[ e->getCar( ) ].back( ) == e );
      assert( uf_to_appl_cache.find( e->getCar( ) ) != uf_to_appl_cache.end( ) );
      uf_to_appl_cache[ e->getCar( ) ].erase( e );
      uf_to_appl[ e->getCar( ) ].pop_back( );
    }
    else
